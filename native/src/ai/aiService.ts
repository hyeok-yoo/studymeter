/**
 * aiService.ts — 앰비언트 AI 상위 서비스. (React Native 포팅)
 *
 * 웹 src/lib/ai/aiService.ts 를 이식한다. 라우터·예산 저장소가 localStorage →
 * AsyncStorage 로 바뀌면서 canSpend/recordSpend/markModelExhausted/markModelCooldown 이
 * 비동기가 됐으므로 해당 호출에 await 를 붙였다 (그 외 로직은 웹과 동일).
 *
...
import type { AiRole, DiaryEntry, DiaryStats, Settings } from '../data/schema';
...
export async function generateForRole(
    ...
    if (!(await canSpend(kind))) return null;
    ...
    for (const model of chain) {
        try {
            ...
            await recordSpend(kind, reply.usedModel);
            return reply;
        } catch (e) {
            if (e instanceof QuotaExceededError) {
                // 일일 소진만 하루 차단. 분당(RPM) 429 는 짧은 쿨다운 후 자동 복구.
                if (e.scope === 'daily') await markModelExhausted(model);
                else await markModelCooldown(model, e.retryAfterMs);
                continue; // 체인 다음 후보로
            }
...
async function generateMorningReportInner(settings: Settings, today: string): Promise<string | null> {
    ...
    return getCachedOrGenerate(kind, today, async () => {
        ...
        const yStr = formatDateYYYYMMDD(yesterday);
...
export async function generateSessionComment(
    ...
    if (!isAmbientAiEnabled(settings)) return null;
    if (!(await canSpend('session-comment'))) return null;
    const today = getTodayDate();
