/**
 * router.ts — AI 역할(role) → 모델 라우팅. (React Native 포팅)
 *
 * 웹 src/lib/ai/router.ts 를 그대로 이식하되 소진 마킹 저장소를
 * localStorage → AsyncStorage 로 바꾼다. AsyncStorage 는 비동기이므로
 * loadExhausted / markModelExhausted / markModelCooldown / isModelExhausted 가
...
import type { AiRole, Settings } from '../data/schema';
...
AsyncStorage.removeItem('studymeter_ai_exhausted').catch(() => { /* ignore */ });
...
async function loadExhausted(): Promise<Record<string, number>> {
    try {
        const raw = await AsyncStorage.getItem(EXHAUSTED_KEY);
        ...
        if (changed) await AsyncStorage.setItem(EXHAUSTED_KEY, JSON.stringify(map));
        return map;
    } catch {
        return {};
    }
...
export async function markModelExhausted(modelId: string): Promise<void> {
    await setExhaustedUntil(modelId, nextPacificMidnight());
}
 ...
 */
export async function markModelCooldown(modelId: string, ms?: number): Promise<void> {
    const cooldown = Math.min(Math.max(ms ?? 60_000, 15_000), 5 * 60_000);
    await setExhaustedUntil(modelId, Date.now() + cooldown);
}

async function setExhaustedUntil(modelId: string, until: number): Promise<void> {
    try {
        const map = await loadExhausted();
        // 이미 더 긴 차단이 걸려 있으면 줄이지 않는다
        map[modelId] = Math.max(map[modelId] ?? 0, until);
        await AsyncStorage.setItem(EXHAUSTED_KEY, JSON.stringify(map));
    } catch { /* ignore */ }
}

export async function isModelExhausted(modelId: string): Promise<boolean> {
    return (await loadExhausted())[modelId] !== undefined;
}
...
export async function buildModelChain(role: AiRole, settings: Settings): Promise<string[]> {
    ...
    const exhausted = await loadExhausted();
    const alive = candidates.filter(id => exhausted[id] === undefined);
