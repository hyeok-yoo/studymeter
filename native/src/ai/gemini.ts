/**
 * gemini.ts — Google Gemini (Generative Language) API 연동 공통 모듈. (React Native 포팅)
 *
 * 웹 src/lib/gemini.ts 를 그대로 이식한다. 이 모듈은 fetch 만 쓰므로(localStorage·DOM
 * 의존 없음) 웹/네이티브 차이가 거의 없다 — RN 의 전역 fetch 를 그대로 사용한다.
 *
